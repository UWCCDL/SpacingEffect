...
USAGE:
    pip install pybads
    python fit_models_slimstampen_bads.py
...
OUTPUT_DIR = '.'
N_RESTARTS = 1     # number of BADS runs per subject per model (best kept)
...
ACTR_PARAMS = {
    ...
    'tau': -0.8,
    's':   0.25,
    #'s': {'bounds': (0.2, 1),   'level': 'subject'},
}

FEAR_PARAMS = {
    'd':   {'bounds': (0.1, 0.75),   'level': 'subject'},
    'w1':  {'bounds': (0, 50),  'level': 'fact'}, # 0-10
    #'t0':  {'bounds': (0.3, 1), 'level': 'subject'},
    #'F':   {'bounds': (0.5, 1.5), 'level': 'subject'},
    't0': 0.3,
    'F': 1,
